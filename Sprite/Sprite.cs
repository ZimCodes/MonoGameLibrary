...
namespace MonoGameLibrary.Sprite
{
    ...
    {
        //Vectors for Location Direction and Origin
        public Vector2 Location, Direction, Origin, Velocity; 
        public float Speed; 
        ...
        /// <summary>
        /// Loads the texture for sprite markers. SpriteMarker no longer a requirement.
        /// </summary>
        private void LoadSpriteMarker()
        {
            Texture2D markers;
            try
            {
                 markers = this.Game.Content.Load<Texture2D>("SpriteMarker");
            }
            catch (Exception e)
            {
                this.SpriteMarkersTexture = null;
            }
        }
        /// <summary>
        ...
        public void DrawMarkers(SpriteBatch sb)
        {
            ...
            if (showMarkers && this.SpriteMarkersTexture != null)
            {
        ...
        /// viewport.</returns>
        protected void clampToViewport(Vector2 vector)
        {
            vp = this.Game.GraphicsDevice.Viewport;
            //Left
            vector.X = MathHelper.Clamp(vector.X, vp.X + this.Origin.X, vp.X + vp.Width);
            //Right
            vector.X = MathHelper.Clamp(vector.X, vp.X, vp.X + vp.Width - this.spriteTexture.Width + this.Origin.X);
            //Top
            vector.Y = MathHelper.Clamp(vector.Y, vp.Y + this.Origin.Y, vp.Y + vp.Height);
            //Bottom
            vector.Y = MathHelper.Clamp(vector.Y, vp.Y, vp.Y + vp.Height - this.spriteTexture.Height + this.Origin.Y);
        }
        /// <summary>
        /// Determine when sprite is currently offscreen
        /// </summary>
        /// <returns>whether sprite is offscreen</returns>
